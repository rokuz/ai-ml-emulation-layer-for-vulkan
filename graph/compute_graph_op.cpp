...
SpecConstants tileConstants(const RescaleTail *tail, const uint32_t inputWords, const uint32_t weightWords) {
...
Conv2D::Conv2D(const std::shared_ptr<VULKAN_HPP_NAMESPACE::detail::DispatchLoaderDynamic> &_loader, VkDevice _device,
               ...
               const uint32_t _tileWeightWords)
    : ComputePipeline(_loader, _device, createDescriptorMap(_input, _output, _weights, _biases, _tail),
                      ...
                      debugName, tileConstants(_tail, _tileInputWords, _tileWeightWords)),
      pushConstant{createPushConstant(_pad, _stride, _dilation, _inputZeroPoint, _weightZeroPoint)},
      maxGroupCount{_maxGroupCount}, tiled{_tileInputWords != 0} {}
...
bool Conv2D::getTileWords(const std::shared_ptr<TensorDescriptor> &input, const std::shared_ptr<TensorDescriptor> &weights,
                          ...
                          const uint32_t sharedMemoryBytes, uint32_t &inputWords, uint32_t &weightWords) {
    ...
    const uint64_t words = (static_cast<uint64_t>(input->getDimensions()[3]) + 3) / 4;
...
Conv3D::Conv3D(const std::shared_ptr<VULKAN_HPP_NAMESPACE::detail::DispatchLoaderDynamic> &_loader, VkDevice _device,
    ...
    : ComputePipeline(_loader, _device, createDescriptorMap(_input, _output, _weights, _biases, _tail),
                      {&pushConstant, sizeof(pushConstant)}, _pipelineCache,
                      createSpirv(_pipelineCache, _input, _output, _weights, _accType, _tail), debugName, rescaleTailConstants(_tail)),
      pushConstant{createPushConstant(_pad, _stride, _dilation, _inputZeroPoint, _weightZeroPoint)} {}
...
SpirvBinary Conv3D::createSpirv(const std::shared_ptr<PipelineCache> &_pipelineCache,
                                const std::shared_ptr<TensorDescriptor> &input,
                                const std::shared_ptr<TensorDescriptor> &output,
                                const std::shared_ptr<TensorDescriptor> &weights, const uint32_t accType, const RescaleTail *tail) const {
    const auto *inType = getFormatInfo(input->getFormat());
    const auto *outType = getFormatInfo(tail != nullptr ? tail->inputFormat : output->getFormat());
    const auto *weightType = getFormatInfo(weights->getFormat());
    const auto *accTypeType = getFormatInfo(accTypeVkFormat(accType));
    ...
    if (tail != nullptr) {
        ...
        return _pipelineCache->lookup(tailShaderName,
                                      {
                                          inType->glslType,
                                          weightType->glslType,
                                          outType->glslType,
...
SpirvBinary DepthwiseConv2D::createSpirv(const std::shared_ptr<PipelineCache> &_pipelineCache,
                                         ...
                                         const std::shared_ptr<TensorDescriptor> &output,
                                         const std::shared_ptr<TensorDescriptor> &weights,
                                         const uint32_t accType, const RescaleTail *tail) const {
    const auto *inType = getFormatInfo(input->getFormat());
    const auto *outType = getFormatInfo(tail != nullptr ? tail->inputFormat : output->getFormat());
    const auto *weightType = getFormatInfo(weights->getFormat());
    const auto *accTypeType = getFormatInfo(accTypeVkFormat(accType));
...
Matmul::Matmul(const std::shared_ptr<VULKAN_HPP_NAMESPACE::detail::DispatchLoaderDynamic> &_loader, VkDevice _device,
               const std::shared_ptr<PipelineCache> &_pipelineCache, const std::shared_ptr<TensorDescriptor> &_input1,
               const std::shared_ptr<TensorDescriptor> &_input2, const std::shared_ptr<TensorDescriptor> &_output,
               const int32_t _inputZeroPoint1, const int32_t _inputZeroPoint2, const std::string &debugName, const RescaleTail *_tail)
...
void GraphPipeline::makeConv2D(const std::shared_ptr<TensorDescriptor> &input,
    ...
    if ((disableTiles == nullptr || std::string_view(disableTiles) == "0") && input->getFormat() == VK_FORMAT_R8_SINT &&
        weights->getFormat() == VK_FORMAT_R8_SINT && accTypeVkFormat(accType) == VK_FORMAT_R32_SINT &&
        sumFormat == VK_FORMAT_R32_SINT && output->getRank() == 4) {
        VkPhysicalDeviceProperties properties;
        loader->vkGetPhysicalDeviceProperties(physicalDevice, &properties);
        ...
        if (channelGroups > properties.limits.maxComputeWorkGroupCount[2] ||
            tileGroupCountX > properties.limits.maxComputeWorkGroupCount[0] ||
            tileGroupCountY > properties.limits.maxComputeWorkGroupCount[1] ||
            !Conv2D::getTileWords(input, weights, stride, dilation, properties.limits.maxComputeSharedMemorySize,
...
void GraphPipeline::makeDepthwiseConv2D(
    ...
    const int8_t inputZeroPoint, const int8_t weightZeroPoint, const uint32_t accType, const std::string &debugName, const RescaleTail *tail) {
